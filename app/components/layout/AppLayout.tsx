      ...
      {manageItems.map((item) => (
        <li key={item.name}>
          <Link
            ...
            className={classNames(
              pathname.includes(`/namespaces/${currentNamespace}${item.href}`)
...
function NamespaceMenu() {
  const { currentNamespace, namespaces } = useContext(NamespaceContext);
  return (
    <>
      ...
      <ul className="-mx-2 mt-2 space-y-1">
        {namespaces.map((namespace) => (
          <li key={namespace.metadata.name}>
            <NamespaceMenuItem
              namespace={namespace}
              active={namespace.metadata.name === currentNamespace}
            />
          </li>
        ))}
      </ul>
